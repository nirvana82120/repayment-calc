...
export function computeAssessment(input, rules) {
  ...
  // === (5) 변제기간: 나이대 기준을 우선 적용 ===
  const ageBand = input?.meta?.ageBand || input?.ageBand || "31-64";
  let months = (rules?.basePeriodByAge && rules.basePeriodByAge[ageBand]) ?? 36;

  // (채무 구간 규칙은 보존만; 다음 단계에서 보정용으로 활용 예정)
  const totalDebt = Math.max(0, Number(input.debts?.total || 0));
  const monthsDebtRule = pickMonthsByDebt(totalDebt, periodByDebt);
  ...
  return {
    ...
    breakdown: {
      ...
      totalDebt,
      adjustedAssets,
      debug: { monthsByAge: months, monthsDebtRule }
    }
  };
}
