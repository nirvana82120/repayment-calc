// app-wire.js — 엔진/룰 자동로드 + 결과 렌더 + consultOnly UI + Google Sheet Webhook(업서트용 세션 포함)
// ===============================================================================================================
const SELF_URL = new URL(import.meta.url);
const V = SELF_URL.searchParams.get('v') || '';
function withV(u){ const url=(u instanceof URL)?new URL(u):new URL(String(u)); if(V) url.searchParams.set('v',V); return url.toString(); }
...
const WEBHOOK_URL = 'https://script.google.com/macros/s/AKfycbzOd6ZNTijgcfkl8egX3ruezIyCjIkB8qiPWJ3CumOJBOW5fkA90MP-BxvxfEFM4mrY0g/exec';
/** ▲▲ */
...
// ---- 세션/타임스탬프 (업서트 키) ----
const SID_KEY='issac_session_id';
const FTS_KEY='issac_first_ts';
...
function getFirstTs(){ return localStorage.getItem(FTS_KEY); }
function setFirstTs(ts){ if(!getFirstTs()) localStorage.setItem(FTS_KEY,String(ts)); }
...
function collectInput(){
  ...
  return {
    ...
    incomes:income, assets, debts, divorce
  };
}

// ---------- Rules/Assessment ----------
async function loadRules(){ const res=await fetch(RULES_URL,{cache:'no-store'}); if(!res.ok) throw new Error('Failed to load rules'); return res.json(); }
...
function buildMetaForWebhook(){
  const meta=collectInput();
  const out={};
  // 평탄화: 시트에서 보기 좋은 1차 필드
  out.home_region = meta.meta.home.region;  out.home_city = meta.meta.home.city;
  out.work_region = meta.meta.work.region;  out.work_city = meta.meta.work.city;
  out.age_band = meta.meta.ageBand;         out.marital = meta.meta.marital;
  return out;
}
...
function ensureRateUI(){
  ...
  if(!row){
    row=document.createElement('div'); row.className='result-item rate';
    const label=document.createElement('span'); label.className='result-label'; label.textContent='변제율';
    const val=document.createElement('span'); val.id='finalRate'; val.className='result-period';
    const unit=document.createElement('span'); unit.className='result-unit'; unit.textContent='%';
    row.appendChild(label); row.appendChild(val); row.appendChild(unit); main.appendChild(row);
    const note=document.createElement('p'); note.id='finalRateNote'; note.className='small-note';
    note.textContent='※ 변제율 = (월×기간) ÷ 무담보총액 × 100, 소수 1자리(0.5% 미만 내림)'; main.appendChild(note);
  }
  return { row, rateEl:$1('#finalRate',row), noteEl:$1('#finalRateNote',row.parentElement) };
}
function showFixedWarnings(){
  const warnWrap=$1('.result-warning'); if(!warnWrap) return;
  let warnBox=warnWrap.querySelector('ul'); if(!warnBox){ warnBox=document.createElement('ul'); warnWrap.appendChild(warnBox); }
  warnBox.innerHTML='';
  ...
  ].forEach(t=>{ const li=document.createElement('li'); li.textContent=t; warnBox.appendChild(li); });
}
...
function renderConsultOnly(out){
  ...
  const {row,noteEl}=ensureRateUI(); if(row) row.style.display='none'; if(noteEl) noteEl.style.display='none';
  const rep=$1('#finalRepayment'); if(rep) rep.textContent=''; const per=$1('#finalPeriod'); if(per) per.textContent='';
}
...
function renderOutput(out){
  __lastResult=out;
  const rep=$1('#finalRepayment'); const per=$1('#finalPeriod');
  if(out?.consultOnly){ renderConsultOnly(out); console.log('[assessment][consultOnly]',out); return; }
  showFixedWarnings();
  ...
  try{
    ...
    if(unsecured>0){
      ...
      let rate=(totalPay/unsecured)*100; rate=Math.floor(rate*10)/10;
      if(rateEl) rateEl.textContent=rate.toLocaleString('ko-KR',{minimumFractionDigits:1,maximumFractionDigits:1});
      if(row) row.style.display='flex'; if(noteEl) noteEl.style.display='';
    }else{ if(row) row.style.display='none'; if(noteEl) noteEl.style.display='none'; }
...
async function calculateAndRender(){
  try{
    const rep=$1('#finalRepayment'); const per=$1('#finalPeriod');
    if(rep) rep.textContent='계산중…'; if(per) per.textContent='';
    ...
    const firstTs=Number(getFirstTs()||Date.now());

    const metaFlat=buildMetaForWebhook();
    const unsecured=Number(out?.breakdown?.debts?.unsecuredTotal||0);
    const rateCalc = unsecured>0 ? (Math.floor(((out.monthlyRepayment*out.months)/unsecured)*1000)/10) : '';
    ...
    await sendWebhook({
      ...
      // 결과 요약
      result:{
        monthlyRepayment:out?.monthlyRepayment||0,
        months:out?.months||0,
        unsecuredTotal:unsecured,
        rate:rateCalc
      },
      // 메타 평탄화
      ...metaFlat
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  new MutationObserver(()=>{ if(!resultSection.hidden) calculateAndRender(); })
    .observe(resultSection,{attributes:true,attributeFilter:['hidden']});
  ...
  submitBtn?.addEventListener('click', async ()=>{
    const contact={
      ...
      preferred_date:$1('#preferredDate')?.value||'',
      preferred_time:$1('#preferredTime')?.value||''
    };
    const out=__lastResult || await runAssessment();
    const sid=getSessionId();
    const firstTs=Number(getFirstTs()||Date.now());
    await sendWebhook({
      event:'consult_submitted',
      session_id:sid,
      first_timestamp:firstTs,
      page_url:location.href,
      referrer:document.referrer||'',
